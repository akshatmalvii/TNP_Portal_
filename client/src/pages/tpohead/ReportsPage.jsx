...
export default function ReportsPage() {
  ...
  useEffect(() => {
    ...
    loadDepartmentScopedFilters(selectedDepartment);
  }, [selectedDepartment]);

  useEffect(() => {
    if (selectedDepartment && selectedSeason) {
      fetchReportData({
  ...
  const loadInitialFilters = async () => {
    ...
    try {
      ...
      const normalizedDepartments = Array.isArray(departmentData) ? departmentData : [];
      setDepartments(normalizedDepartments);
      setError("");

      if (normalizedDepartments.length > 0) {
        setSelectedDepartment(String(normalizedDepartments[0].dept_id));
      } else {
        setLoading(false);
      }
    } catch (err) {
  ...
  const loadDepartmentScopedFilters = async (departmentId) => {
    setLoading(true);
    try {
      const [coursesRes, seasonsRes] = await Promise.all([
        fetch(`${API_BASE}/courses?dept_id=${departmentId}`, {
          headers: getHeaders(),
        }),
        fetch(`${API_BASE}/tpo/head/reports/seasons?dept_id=${departmentId}`, {
          headers: getHeaders(),
        }),
      ]);

      if (!coursesRes.ok) throw new Error("Failed to load courses");
      if (!seasonsRes.ok) throw new Error("Failed to load placement seasons");
      ...
      const normalizedSeasons = Array.isArray(seasonData) ? seasonData : [];

      setCourses(normalizedCourses);
      setSeasons(normalizedSeasons);
      setSelectedCourse("");
      setSelectedSeason((currentSeason) =>
        normalizedSeasons.includes(currentSeason) ? currentSeason : normalizedSeasons[0] || ""
      );
      setError("");
  ...
  const fetchReportData = async ({ season, departmentId, courseId = "" }) => {
    setLoading(true);
    try {
      ...
      const res = await fetch(`${API_BASE}/tpo/head/reports/${season}?${query.toString()}`, {
        headers: getHeaders(),
      });
  ...
  const handleExport = async () => {
    if (!selectedSeason || !selectedDepartment) return;
    ...
    try {
      ...
      const res = await fetch(
        `${API_BASE}/tpo/head/reports/${selectedSeason}/download?${query.toString()}`,
        {
          headers: getHeaders(),
        }
      );
  ...
  const selectedDepartmentName =
    reportData?.department?.dept_name ||
    departments.find((department) => String(department.dept_id) === selectedDepartment)?.dept_name ||
    "Department";

  const selectedCourseName =
    reportData?.course?.course_name ||
    courses.find((course) => String(course.course_id) === selectedCourse)?.course_name ||
    "All Courses";

  if (error) return <ErrorState message={error} onRetry={loadInitialFilters} />;
    ...
    <div className="p-6 space-y-6">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">
            Year-wise reports across departments and courses
          </p>
        </div>
        ...
        <div className="flex flex-col lg:flex-row items-start lg:items-center gap-3">
          <select
            ...
            className="border-gray-300 rounded-md shadow-sm bg-white px-4 py-2 text-sm focus:ring-indigo-500 font-medium min-w-[170px]"
          >
          ...
          <select
            ...
            className="border-gray-300 rounded-md shadow-sm bg-white px-4 py-2 text-sm focus:ring-indigo-500 font-medium min-w-[220px]"
          >
          ...
          <button
            ...
            disabled={!selectedSeason || !selectedDepartment || exporting || loading}
      ...
      {loading ? (
        ...
        <div className="text-center py-12 text-gray-400 bg-white rounded-lg shadow-sm">
          Select a year and department to view records
        </div>
      ) : (
        <>
                ...
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      Out of {reportData.totalStudents} Registered
                    </p>
                  </div>
              ...
              <CardContent className="pt-6">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 mt-1">
                      {selectedDepartmentName} in {selectedSeason}
                    </p>
                  </div>
          ...
          <Card className="border-0 shadow-sm">
            <CardHeader className="border-b pb-4">
              ...
              <p className="text-sm text-gray-500">
                Full audit log for {selectedDepartmentName} ({selectedCourseName}) in {selectedSeason}.
              </p>
            </CardHeader>
                  ...
                  {reportData.placedStudents.length === 0 ? (
                    ...
                    reportData.placedStudents.map((ps, idx) => (
                      <tr key={idx} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-4">
                          <p className="font-medium text-gray-800">{ps.dept_name}</p>
                          <p className="text-xs text-gray-500">{ps.course_name || "All Courses"}</p>
                        </td>
                        <td className="px-4 py-4">
                        ...
                        <td className="px-4 py-4">
                          ...
                          {ps.stipend_pm ? (
                            <p className="text-xs text-indigo-600 bg-indigo-50 px-1.5 py-0.5 rounded inline-block mt-1">
                              Stipend: {ps.stipend_pm}
                            </p>
                          ) : null}
                        </td>
